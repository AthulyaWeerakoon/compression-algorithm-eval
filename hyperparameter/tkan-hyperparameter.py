...
def build_kan_width(config):
    ...
    width = [INPUT_STEPS]  # Input size fixed
    
    # Add hidden layers only if they have positive values
    for layer_name in ['hidden_layer1', 'hidden_layer2', 'hidden_layer3']:
        if hasattr(config, layer_name):
            layer_size = getattr(config, layer_name)
            if layer_size > 0:
                width.append(layer_size)
    
    width.append(OUTPUT_STEPS)  # Output size fixed
    return width
